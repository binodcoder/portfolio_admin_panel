...
class _CertificationFormState extends ConsumerState<CertificationForm> {
  ...
  Future<void> _save() async {
    ...
    final data = Certification(
      ...
      issueDate: issueDateController.text.trim().isEmpty ? null : issueDateController.text.trim(),
      expiryDate: expiryDateController.text.trim().isEmpty ? null : expiryDateController.text.trim(),
      credentialId: credentialIdController.text.trim().isEmpty ? null : credentialIdController.text.trim(),
      credentialUrl: credentialUrlController.text.trim().isEmpty ? null : credentialUrlController.text.trim(),
    );
                    ...
                    Row(children: [
                      ...
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Issue (YYYY-MM)'),
                        ),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Expiry (YYYY-MM, optional)'),
                        ),
                      ),
                    ]),
                    const SizedBox(height: 12),
                    Row(children: [
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Credential ID (optional)'),
                        ),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
